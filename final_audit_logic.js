...
const osrmData = JSON.parse(fs.readFileSync('routes.json', 'utf8'));
...
for (let i = 0; i < osrmData.routes.length; i++) {
    ...
    for (const [lng, lat] of sampled) {
        ...
        const res = JSON.parse(require('child_process').execSync(`curl -s "${url}"`).toString());
        if (res.features) res.features.forEach(f => {
            ...
            if (c.some(x => x.includes('public_transport') || x.includes('transport') || x.includes('bus') || x.includes('train') || x.includes('railway'))) {
                ...
                if (!all.has(f.properties.place_id)) {
                    all.set(f.properties.place_id, { ...f.properties, s: sampled.indexOf([lng,lat])+1 });
                }
            }
        });
    }
    ...
    console.log(`\n--- ROUTE ${i+1} ---`);
    console.log(`Transit: ${list.length}`);
    list.forEach((p, idx) => {
        console.log(`[R${i+1}_REC ${idx+1}] ${p.name} | ${p.place_id} | ${p.lat},${p.lon} | ${p.categories.join(', ')}`);
    });
