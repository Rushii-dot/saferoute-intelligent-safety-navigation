...
async function audit() {
    ...
    for (let i = 0; i < osrmData.routes.length; i++) {
        ...
        const counts = { police: 0, hospitals: 0, pharmacies: 0, transit: 0, schools: 0, parks: 0, shops: 0, fuelStations: 0 };
        const transitPlaces = [];
        ...
        allPlaces.forEach(p => {
            ...
            if (cats.some(c => c.includes('pharmacy'))) counts.pharmacies++;
            if (cats.some(c => c.includes('school') || c.includes('university') || c.includes('college'))) counts.schools++;
            if (cats.some(c => c.includes('park') || c.includes('garden'))) counts.parks++;
            ...
            if (cats.some(c => c.includes('fuel'))) counts.fuelStations++;
        ...
        const emergencyScore = 25 * (1 - Math.exp(-emergencyDensity / 0.5));

        const finalScore = Math.round(roadScore + activityScore + emergencyScore);

        console.log(`\nROUTE ${i + 1} AUDIT (Talegaon -> Pune)`);
        console.log(`Distance: ${distanceKm.toFixed(2)} km`);
        console.log(`Duration: ${(route.duration / 60).toFixed(1)} mins`);
        ...
        console.log(`- Emergency Score: ${emergencyScore.toFixed(1)} /25`);
